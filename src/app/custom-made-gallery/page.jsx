...
export default function CustomMadeGalleryPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col metallic-base-bg overflow-auto">
      <div className="w-full max-w-5xl mx-auto px-4 pt-24 pb-8 flex flex-col items-end">
        ...
        {loading ? (
          <div className="w-full text-center text-gray-400 py-16">불러오는 중...</div>
        ) : error ? (
          ...
          <div className="w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-16">
            {gallery.map((item) => (
              <div
                key={item.id}
                className="relative group aspect-square overflow-hidden shadow-md bg-white/40 cursor-pointer"
